...
for location in "${locations[@]}"; do
  location_resources=$(az resource list --subscription "$SUBSCRIPTION_ID" --resource-group "$RESOURCE_GROUP" --location "$location" --query "[].{location:location,type:type}" -o json 2>&1) || continue
  ...
  if [ "$(echo "$location_resources" | jq -r '. | length')" -gt 0 ]; then
    for resource in $(echo "$location_resources" | jq -c '.[]'); do
      ...
      if [[ "$exclude" == false ]]; then
        filtered_resources+=("$resource_location")
      fi
    done
  fi
done
